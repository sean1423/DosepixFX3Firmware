/*
 ## Cypress USB 3.0 Platform source file (cyfxUSBTOGPIF.c)
 ## ===========================
 ##
...
// 18/07/16
// Implemented DMA manual mode for USB->GPIF->USB. at this stage how it works:
// if STREAMING_MANUAL is enabled:
...
extern uint8_t glFirmwareID[32];
extern CyU3PReturnStatus_t CyFxSpiTransfer (uint16_t , uint16_t , uint8_t , CyBool_t  );


uint8_t glEp0Buffer[4096] __attribute__ ((aligned (32))); /* Local buffer used for vendor command handling. */ // Changed this to glEp0Buffer from glEp0Buffer in order to avoid conflict. Still to see if this works. Probably not since it is regarding vendor commands.
//Commented the above line out in order to
...
static volatile CyBool_t gDoLpmDisable = CyFalse; /* Variable to check whether LPM transitions should be disabled. */
        ...
        CyU3PDmaCBInput_t *input)
{
    ...
    if (type == CY_U3P_DMA_CB_PROD_EVENT)
    {
    	...
    	CyU3PDebugPrint(4, "\n\rUSBTOGPIFDmaCallback :: %d bytes transfered", input->buffer_p.count);
        ...
        //uint8_t out = 0;
        //out = CyU3PMemCmp(input->buffer_p.buffer, command, 1);
        //CyU3PDebugPrint(4, "\n\rUSBTOGPIFDmaCallback :: comparison result %d", out);
        ...
        CyU3PDmaCBInput_t *input)
{
    ...
    if (type == CY_U3P_DMA_CB_PROD_EVENT)
    {
    	CyU3PDebugPrint(4, "\n\rGPIFTOUSBDmaCallback :: %d bytes transfered", input->buffer_p.count);
        /* Change the first and last data bytes for tracking on a USB trace. */
        //input->buffer_p.buffer[0x0000] = DataSignature++;
        //input->buffer_p.buffer[input->buffer_p.count - 1] = DataSignature++;
        ...
        uint8_t           epNum)
{
    ...
    if (evtype == CYU3P_USBEP_SS_RESET_EVT)
    {
        if (epNum == CY_FX_USB_PRODUCER)
        {
            ...
            CyU3PUsbStall (CY_FX_USB_PRODUCER, CyTrue, CyFalse);
        }
        if (epNum == CY_FX_EP_LOOP_IN)
        {
        ...
        void)
{
    ...
    //Defining Debug Endpoint
        CyU3PMemSet ((uint8_t *)&epCfg, 0, sizeof (epCfg));
            ...
            /* USB debugger endpoint configuration */
    ...
    apiRetStatus = CyU3PSetEpConfig (CY_FX_USB_PRODUCER, &epCfg);
    if (apiRetStatus != CY_U3P_SUCCESS)
    {
        CyU3PDebugPrint (4, "CyU3PSetEpConfig failed, Error code = %d\n", apiRetStatus);
        CyFxAppErrorHandler (apiRetStatus);
    ...
    /* Flush the endpoint memory */
    CyU3PUsbFlushEp(CY_FX_USB_CONSUMER);

    // Loop-back end points
    ...
    dmaCfg.prodSckId = CY_FX_USB_PRODUCER_SOCKET;
    ...
    dmaCfg.cb = USBTOGPIFDmaCallback;
    ...
    apiRetStatus = CyU3PDmaChannelSetXfer (&glDmaChHandle, CY_FX_USBTOGPIF_DMA_TX_SIZE);
    if (apiRetStatus != CY_U3P_SUCCESS)
    {
    ...
    dmaCfg.consSckId = CY_FX_USB_CONSUMER_SOCKET;
    dmaCfg.dmaMode = CY_U3P_DMA_MODE_BYTE;
    dmaCfg.prodHeader = 0;
    dmaCfg.prodFooter = 0;
    dmaCfg.consHeader = 0;
    ...
    apiRetStatus = CyU3PDmaChannelCreate (&glDmaChHandle1, CY_U3P_DMA_TYPE_MANUAL, &dmaCfg);
#else
    dmaCfg.notification = CY_U3P_DMA_CB_CONS_SUSP;
    //dmaCfg.cb = GpifToUsbDmaCallback;
    apiRetStatus = CyU3PDmaChannelCreate (&glDmaChHandle, CY_U3P_DMA_TYPE_AUTO, &dmaCfg);
    ...
    /* Set DMA Channel transfer size */
    apiRetStatus = CyU3PDmaChannelSetXfer (&glDmaChHandle1, CY_FX_GPIFTOUSB_DMA_TX_SIZE);
    if (apiRetStatus != CY_U3P_SUCCESS)
    {
        CyU3PDebugPrint (4, "CyU3PDmaChannelSetXfer failed, Error code = %d\n", apiRetStatus);
        CyFxAppErrorHandler(apiRetStatus);
    ...
    )
{
    ...
    if (bType == CY_U3P_USB_VENDOR_RQT) //Checks bType is Vendor. On Control Centre if you want to use these Vendor Commands then always select "Vendor".
    {
        ...
        { //The cases for vendor commands here are for SPI. These all work, however
            ...
            case CY_FX_RQT_SPI_FLASH_WRITE: // This case is taken from SPI.c and the function it was copied from was commented out for testing.
            ...
            case CY_FX_RQT_SPI_FLASH_READ: // This case is taken from SPI.c and the function it was copied from was commented out for testing.
            ...
            case CY_FX_RQT_SPI_FLASH_ERASE_POLL:// This case is taken from SPI.c and the function it was copied from was commented out for testing.
    ...
    if (bType == CY_U3P_USB_STANDARD_RQT)
    {
                ...
                && (wValue == CY_U3P_USBX_FS_EP_HALT))
        {
            if (glIsApplnActive)
            {
                if (wIndex == CY_FX_USB_PRODUCER)
                {
                    CyU3PDmaChannelReset (&glDmaChHandle);
                    CyU3PUsbFlushEp(CY_FX_USB_PRODUCER);
                    CyU3PUsbResetEp (CY_FX_USB_PRODUCER);
                    CyU3PDmaChannelSetXfer (&glDmaChHandle, CY_FX_USBTOGPIF_DMA_TX_SIZE);
...
{
    CyU3PReturnStatus_t apiRetStatus = CY_U3P_SUCCESS;
    CyU3PPibClock_t pibClk = {20, CyFalse, CyFalse, CY_U3P_SYS_CLK};
        ...
        uint32_t input)
{
    ...
    for (;;)
    {
        ...
        else
        {
            ...
            if ((gDoLpmDisable) && (CyU3PUsbGetSpeed () == CY_U3P_SUPER_SPEED))
            {
                /* If the link is in U1/U2 states, try to get back to U0. */
                stat = CyU3PUsbGetLinkPowerState (&curState);
                if ((stat == CY_U3P_SUCCESS) && (curState >= CyU3PUsbLPM_U1) && (curState < CyU3PUsbLPM_U3))
                {
        ...
        void)
{
    ...
    ptr = CyU3PMemAlloc (CY_FX_USBTOGPIF_THREAD_STACK);

    /* Create the thread for the application */
    ret = CyU3PThreadCreate (&glAppThread,                      /* App thread structure */
                          "21:DOSEPIX",                     	/* Thread ID and thread name */
                          CyFxAppThread_Entry,                  /* App thread entry function */
                          0,                                    /* No input parameter to thread */
                          ptr,                                  /* Pointer to the allocated thread stack */
                          CY_FX_USBTOGPIF_THREAD_STACK,         /* App thread stack size */
                          CY_FX_USBTOGPIF_THREAD_PRIORITY,      /* App thread priority */
                          CY_FX_USBTOGPIF_THREAD_PRIORITY,      /* App thread priority */
...
// Here the IO matrix for the FX3 device is configured.
    ...
    io_cfg.useSpi    = CyFalse; // Change to CyTrue to enable SPI
    //io_cfg.lppMode   = CY_U3P_IO_MATRIX_LPP_DEFAULT;
    io_cfg.lppMode   = CY_U3P_IO_MATRIX_LPP_UART_ONLY; // as per cyfxbulklpauto.c //Change to CY_U3P_IO_MATRIX_LPP_DEFAULT if SPI is in use
